...
const float correctionFactor = 0.93;  // Scaling factor to fix the 90-degree issue (adjust if needed)
...
void setup() {
  ...
  prevTime = micros(); // Initialize time using micros() for better precision
}
...
void loop() {
  ...
  // Optional: Apply the correction factor for tilt beyond 90 degrees (optional, adjust if needed)
  calibratedPitch *= correctionFactor;

  // **Apply an additional offset correction if the sensor is reading -0.4 degrees on flat ground**
  // Example: you want to subtract -0.4° to make it closer to 0°.
  ...
  // Remove delay for faster updates
  // delay(10); // Removed delay
}
